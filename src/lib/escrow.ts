...
export const ESCROW_SOURCE = `# { "Depends": "py-genlayer:1jb45aa8ynh2a9c9xn3b7qqh8sm5q93hwfp7jqmwsfhh8jpz09h6" }
...
export type EscrowEvidence = { submitter: string; role: string; content: string }

function withTimeout(p: Promise<any>, ms: number): Promise<any> {
  return Promise.race([p, new Promise((_r, rej) => setTimeout(() => rej(new Error("read timeout")), ms))])
}

export async function readEscrowStatus(address: string): Promise<{ status: EscrowStatus; evidence: EscrowEvidence[] }> {
  const rc = readClient()
  const rawStatus: any = await withTimeout(rc.readContract({ address, functionName: "get_status", args: [], stateStatus: "accepted" }), 15000)
  let evidence: EscrowEvidence[] = []
  try {
    const rawEv: any = await withTimeout(rc.readContract({ address, functionName: "get_evidence", args: [], stateStatus: "accepted" }), 15000)
    const parsed = typeof rawEv === "string" ? JSON.parse(rawEv) : rawEv
...
export async function deployEscrow(client: any, seller: string, amountWei: bigint, terms: string): Promise<{ hash: string; address: string }> {
  const code = new TextEncoder().encode(ESCROW_SOURCE)
  const hash: string = await client.deployContract({ code, args: [seller, amountWei, terms] })
  const rc = readClient()
  try { await rc.waitForTransactionReceipt({ hash, status: TransactionStatus.ACCEPTED, interval: 3000, retries: 80 }) } catch {}
  let address = ""
  for (let i = 0; i < 40 && !address; i++) {
    try {
      const tx: any = await rc.getTransaction({ hash })
      address = (tx && tx.txDataDecoded && tx.txDataDecoded.contractAddress) || (tx && tx.recipient) || ""
    } catch {}
    if (!address) await new Promise((r) => setTimeout(r, 3000))
  }
  if (!address) throw new Error("Deployed (tx " + hash + ") but address not available yet. Try Load in a moment.")
  return { hash, address }
}
